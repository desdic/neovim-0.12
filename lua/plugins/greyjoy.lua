            ...
            commands = {
                ...
                ["cmake --build build"] = {
                    ...
                    condition = function(n)
                        return condition.file_exists("CMakeLists.txt", n) and condition.directory_exists("build", n)
                    end,
                },
                ["cmake -S . -B build"] = {
                    command = { "cd", "{rootdir}", "&&", "cmake", "-S", ".", "-B", "{rootdir}/build" },
                    condition = function(n)
                        return condition.file_exists("CMakeLists.txt", n) and not condition.directory_exists("build", n)
                    end,
                },
                ["build-login"] = {
...
vim.keymap.set("n", "<leader>gr", "<cmd>Greyjoy<CR>", { desc = "[G]reyjoy [r]un" })
vim.keymap.set("n", "<leader>gg", "<cmd>Greyjoy fast<CR>", { desc = "[G]reyjoy fast [g]roup" })
vim.keymap.set("n", "<leader>rl", "<cmd>GreyjoyRunLast<CR>", { desc = "Run last greyjoy" })
